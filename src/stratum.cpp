...
bool parse_mining_notify(String line, mining_job& mJob)
{
    ...
    mJob.test = String((const char*) doc["params"][5]);

    // Extrahiere die Blockhöhe aus dem Blockheader
   // if (true){//(mJob.version.startsWith("00000001")) {
        //mJob.block_height = strtoul(mJob.version.c_str() + 8, NULL, 16);
        
        //Serial.print("    Blockhöhe: "); Serial.println(mJob.block_height);
   // }
